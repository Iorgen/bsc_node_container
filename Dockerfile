...
ARG NETWORK
RUN echo "$VERSION"
RUN wget https://github.com/binance-chain/bsc/releases/download/v${VERSION}/geth_linux
RUN wget https://github.com/binance-chain/bsc/releases/download/v${VERSION}/${NETWORK}.zip


#RUN curl --silent "https://api.github.com/repos/binance-chain/bsc/releases/tags/v${VERSION}" | jq -c ".assets[] | select( .browser_download_url | contains($NETWORK.zip) or contains('geth_linux')) | .browser_download_url" | xargs -n1 curl -LOJ && \
